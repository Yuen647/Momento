...
public final class ParamUtils {
     ...
     * @return
     */
    public static boolean checkMomentoId(String momentoId) {
        // 检查长度
        if (momentoId.length() < ID_MIN_LENGTH || momentoId.length() > ID_MAX_LENGTH) {
            return false;
        }
        // 检查格式
        Pattern pattern = Pattern.compile(ID_REGEX);
        return pattern.matcher(momentoId).matches();
    }
